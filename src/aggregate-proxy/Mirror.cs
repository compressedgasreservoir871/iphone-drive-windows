...
internal class Mirror : IDokanOperations2
{
    ...
    public Mirror(ILogger logger, string path, long totalBytes, long freeBytes)
    {
...
#pragma warning restore CA2002 // Do not lock on objects with weak identity
    ...
    {
        volumeLabel.SetString("iPhone");
        fileSystemName.SetString("AFC");
    ...
    {
        security = null;
        return Trace(nameof(GetFileSecurity), fileName, info, DokanResult.NotImplemented, sections.ToString());
    }
    ...
    public IEnumerable<FindFileInformation> FindFilesHelper(ReadOnlyNativeMemory<char> fileName, string searchPattern)
    {
        var files = new DirectoryInfo(GetPath(fileName))
            ...
            .Where(finfo => !finfo.Name.Equals("$RECYCLE.BIN", StringComparison.OrdinalIgnoreCase) && !finfo.Name.Equals("desktop.ini", StringComparison.OrdinalIgnoreCase))
